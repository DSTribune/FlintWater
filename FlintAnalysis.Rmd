...
tapply(percentConc$Median, percentConc$County, mean)
```
We have finally arrived closer to the truth. In general the rivers and lakes in Genesee County appear to have a much lower chloride concentration than those in Wayne County. So wouldn't this put Flint, Mi at an advantage when sourcing their water locally? To explore this question we need to get more granular than the county. We need the exact coordinates of where the input for Flint's water is coming from.

Further research shows that Flint is source their water from the Holloway Reservoir. 
